...
최초 구현 기준 `aef949c`는 운영 리비전 `qr-order-staging-00010-kzm`보다 이전 기능을 포함해 직접 배포하지 않았다. 2026-09-17에는 `origin/dev`의 `00428f6`을 통합하고 회귀 시험을 통과했다. 운영 배포·용량 재검증은 아직 수행하지 않았다. 작업 중 운영 DB는 연결 예산 확인을 위해 읽기만 수행했다. 추가 운영 주문·부하 시험, Cloud Run 설정 변경, Git push는 수행하지 않았다.
...
1. 완료(2026-09-17): `origin/dev`의 `00428f6` 통합 및 회귀 테스트. 배포 시점에 추가 변경이 생기면 다시 확인한다.
...
## 2026-09-17 dev 병합 검증

- `origin/dev` (`00428f6`)의 메뉴 삭제·엽전 설정, 개별 조리/서빙, 공통 메뉴 이미지 표시, 메뉴 탐색/테이블 배치 개선을 통합했다.
- 주문번호 할당기와 조리 초기화를 함께 주입하고 중복 잠금/필드를 정리했다. 카테고리·스크롤 유지와 고객 유휴 처리를 함께 보존했다.
- 공통 `MenuImage`의 로딩 실패 대체 표시와 소주·맥주 기본 이미지의 흰 배경/전체 병 표시를 결합했다.
- 새 조리/서빙 큐와 부하 시험을 함께 실행하자 이벤트별 DB 연결 획득 때문에 SSE 전달 지연이 재현됐다. 리스너가 알림을 받으면 커밋된 이벤트를 250개 단위로 읽도록 바꿔 연결 획득 횟수를 줄였다. 테스트 고객도 실제 화면처럼 SSE를 계속 읽도록 했다.
- 백엔드 41개, 프런트엔드 34개, 운영 도구 5개 테스트 및 frontend lint/build·Terraform validate 통과.
- 고객 100개·직원 4개 연결 조건에서 40명/50명 주문 각 3회 모두 성공. 50명 주문+50개 조회의 주문 p95 257ms, 조회 p95 309ms. 합성 DB 지연 100ms 조건도 50/50 성공.
- 주문 이벤트 420개가 직원 스트림 4개에 각각 순서대로 모두 전달됐고, 고객 연결 40개 종료 후 서버 등록 수가 100→60으로 감소했다.
- 위 수치는 개발 머신의 격리 DB 결과이며 운영 성능 보장은 아니다. 앞 절의 2026-09-16 측정값은 당시 기록으로 남긴다.
