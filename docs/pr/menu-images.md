...
투명 배경 메뉴 일러스트 13개와 소주·맥주·콜라·사이다 사진 4개를 기존 메뉴에 연결했습니다. 가로로 넓은 음식과 세로로 긴 음료 그림을
...
- 배경 제거 결과를 포함한 입력 PNG 17개를 크기·비율·알파 채널을 보존한 WebP로 변환했습니다.
  현재 사용하는 파일의 총 용량은 1,089,608 bytes입니다. 사진 4개의 배경은 투명합니다.
- `assets/menu/manifest.json`과 내용 해시 파일명으로 메뉴별 매핑 및 버전 관리.
- Terraform에 이미지 전용 서울 Standard 버킷, 공개 읽기 IAM, 캐시 메타데이터를 가진 객체를 추가했습니다.
  현재 메뉴 17개와 롤백용 소주·맥주 이미지 2개를 포함해 총 19개 객체를 유지합니다.
...
기존 화면의 배치·크기·토큰을 유지했습니다. 이미지 소스는 `메뉴판(최종)`의 PNG 13개와 추가 제품 사진 4개입니다.
...
### 음료 4종 투명 배경 적용 (2026-09-17)

- 콜라·사이다를 새로 연결하고 기존 소주·맥주를 투명 배경 버전으로 교체했습니다.
- 내장 ImageGen 편집 결과 PNG와 사용 프롬프트는 [menu-cutouts](../../assets/menu-cutouts/README.md)에 보관했습니다.
  생성형 편집 결과로 원본 사진의 픽셀을 그대로 추출한 파일은 아닙니다.
- 네 PNG에서 실제 alpha 투명 영역과 불투명 제품 영역을 확인했습니다. WebP 변환에서도 알파 채널을 보존했습니다.
- 편집 PNG 4개 4,480,882 bytes → 배포 WebP 510,362 bytes.
- Terraform plan은 새 객체 4개 생성만 포함했습니다. 기존 흰 배경 소주·맥주는 롤백용으로 보관합니다.
- 기존 다른 메뉴 이미지 13개가 변하지 않았음을 확인했습니다.
- GCS 익명 다운로드·Content-Type·SHA-256을 검증하고, DB 변경 계획에서 소주·맥주 교체와 콜라·사이다 추가만 확인했습니다.
- 배포 앱에 테스트 카탈로그를 주입해 320px/390px에서 네 메뉴의 목록·상세와 소주·맥주 이벤트 이미지를 확인했습니다.
  흰 사각 배경 없이 표시되며 수평 overflow와 page error가 없습니다. 실제 주문 요청은 실행하지 않았습니다.
