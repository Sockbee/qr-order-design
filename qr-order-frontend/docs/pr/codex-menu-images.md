...
메뉴에 사용자가 제공한 투명 배경 일러스트 13개를 연결했습니다. 기존 `object-cover`로 잘리던
음식·음료 그림을 전체 비율로 표시하고, Cloud Storage에서 앱과 독립적으로 배포합니다.
...
- [메뉴 이미지 운영 가이드](../../../docs/qr-order/menu-images.md) 및 아키텍처 문서 갱신.

## Figma

기존 화면의 배치·크기·토큰을 유지했습니다. 이번 변경의 이미지 소스와 표시 방식은 사용자가 제공한
`메뉴판(최종)` 파일 및 승인한 구현 방향을 따릅니다. Figma 파일은 수정하지 않았으며 새 프레임 비교는 수행하지 않았습니다.
