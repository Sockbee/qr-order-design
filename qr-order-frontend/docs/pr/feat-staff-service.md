...
화면은 둘이다 — **A10 서비스 지급**(테이블 단위)과 **B04 서비스 정산**(스태프 단위).
...
A02 인스펙터 패널에 `서비스` 버튼을 추가해 지급 화면 진입점을 만들었다.
`StaffTableOperationRoute`의 다이얼로그 패턴이 아니라 `StaffAddOrderRoute`와 같은 전체 화면
라우트다 — 메뉴 그리드가 필요하고, 부담 스태프와 사유라는 필수 결정이 둘 더 붙기 때문이다.
...
- 렌더링 검증 — 아이패드 뷰포트 1194×834에서 두 화면을 실제로 띄워 확인했다. 가로 overflow
  없음, 콘솔 에러 없음.
...
원래 값으로 복원했다. 이 파일은 git 추적 대상이 아니며 커밋에 포함되지 않는다.
...
- **A10을 `StaffAddOrderPage` 재사용이 아니라 별도 페이지로 만들었다.** A03의 props가
  `tableId/draft/note/onSubmit` 고정인데 여기에 `chargedStaffId`·`serviceReason`·부담액
