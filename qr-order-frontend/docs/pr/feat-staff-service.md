...
| `/staff/availability` | B05 — 품절 관리 |

`/staff/tables/:tableId/order`(A03 주문 추가)는 **삭제했다.**
...
A02에서 진입점이 사라진 A03 — Add Order를 지우고, 거기 얹혀 있던 `품절 관리`만 독립
화면으로 떼어냈다.

**삭제한 것**

- `pages/staff/StaffAddOrderPage.tsx` / `.css`
- `StaffOperationRoutes.tsx`의 `StaffAddOrderRoute`와 `OrderDraftLine`
- `api/staff/menu.ts`의 `createStaffOrder` / `StaffOrderDraftLine` — 호출자가 사라졌다.
  `staff/orders/create` 계약 자체는 문서 §1에 남아 있고, 필요해지면 히스토리에서 되살릴 수
  있다.
- 스태프 번들이 100.80 kB → 97.24 kB로 줄었다.

**새로 만든 것**

| 파일 | 역할 |
|---|---|
| `pages/staff/StaffAvailabilityPage.tsx` | B05 본체 |
| `pages/staff/StaffAvailabilityRoute.tsx` | `useStaffMenu` 연결 |

- `AvailabilityCard`(`components/staff/StaffMenuCard.tsx`)는 그대로 재사용했다. 원탭 전환,
  확인 단계 없음이라는 성격도 유지했다.
- 셸은 `StaffStationPage`를 재사용한다. 새 페이지 셸을 만들지 않았다.
- **한 덩어리 그리드가 아니라 `품절` / `판매 중` 두 섹션으로 나눴다.** 이 화면이 답해야 할
  질문이 "지금 뭐가 품절인가"이고, 스위치를 누르면 카드가 두 섹션 사이를 오간다 — B03의
  결제 대기/완료, B04의 미정산/정산 완료와 같은 형태다. `품절`을 위에 둔 것은 그쪽이 확인할
  대상이기 때문이다.
- 카탈로그는 A10과 같은 `useStaffMenu`를 쓴다. 두 화면이 품절 상태를 두고 어긋날 수 없다.
...
`StaffNavigation` 항목이 5개에서 7개가 됐다. 순서는
`테이블 · 주방 · 서빙 · 결제 · 서비스 · 품절 · 설정`이다. `설정`은 주 워크플로 바깥이라
계속 마지막이다.

`품절`에는 배지를 달지 않았다. 배지를 달려면 모든 화면이 카탈로그를 읽어야 하는데 그런
화면이 없어서, 자기 화면에만 나타나는 숫자가 된다. 개수는 그 화면 헤더가 보여준다.
...
| B05 — Sold-out Management | `228:1805` | x=18236 |
...
기존 프레임 중 `staff/TableDetailPanel`(`89:8`)의 주 버튼 라벨을 `서비스 제공`으로 바꿨다.
컴포넌트라서 A02 프레임(`90:183`)에 자동 반영된다. A10·B04 레일에도 `품절` 항목을 넣어
7개로 맞췄다.
...
- **A10에는 확인 다이얼로그가 있다. A03에는 없다.** A03은 "담은 항목 패널이 곧 확인이고 모든
  ...
  필요하다고 판단했다.
- **A10에 `품절 관리`를 넣지 않았다.** 이제 B05가 그 화면이고, 같은 스위치를 두 화면에서
  뒤집을 수 있게 만들 이유가 없다.
...
- **Figma `staff/StaffNavigation` 컴포넌트 세트에 `서비스`·`품절` 행이 없다.** 새 프레임
  세 장은 7개짜리 레일을 로컬로 그렸고 공유 컴포넌트를 인스턴스하지 않았다. 공유 세트를
  고치면 모든 variant에 행 두 개를 추가해야 하고 기존 프레임 15장의 레일이 함께 바뀌므로,
  이번 범위를 넘는다고 판단했다. 컴포넌트 세트 갱신은 별도 작업으로 남긴다.
...
- 지급 화면은 옵션 선택을 지원하지 않는다. 삭제된 A03도 같은 수준이었다.
- **운영진 대행 주문 기능이 앱에서 사라졌다.** 손님이 각자 주문한다는 전제이며, 대행 주문이
  다시 필요해지면 A03을 히스토리에서 되살리는 것이 아니라 요구사항을 다시 정의하는 편이
  맞다.
- B05에는 검색·분류 필터가 없다. 삭제된 A03의 품절 관리 모드도 없었고, 두 섹션으로 나뉘어
  품절 목록이 짧게 유지되므로 지금 메뉴 수(26개)에서는 필요하지 않다고 봤다.
- **`품절 관리`를 설정 화면에 합치지 않았다.** 설정에도 메뉴별 `판매` 체크박스가 있지만
  그건 행마다 저장 버튼을 눌러야 하는 관리자 폼이다. B05는 서비스 중 서서 한 번 탭하는
  용도라 필요한 상호작용이 다르고, 둘을 합치면 둘 중 하나가 나빠진다. 같은 값을 두 곳에서
  바꿀 수 있다는 점은 감수한다 — 둘 다 `menu/availability`를 호출한다.
